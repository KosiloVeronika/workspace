        ...
        <fieldset>
            <form method="POST" action="${pageContext.servletContext.contextPath}/Controller">
	            ...
	            <input type="hidden" name="user_id" value="${user.userID }" />
					...
					<input type="radio" name="isPensioner" <c:if test="${user.isPensioner == 'YES'}"> checked="checked"</c:if> value="YES" />Пенсионер
	            	<input type="radio" name="isPensioner" <c:if test="${user.isPensioner == 'NO'}"> checked="checked"</c:if> value="NO" />Не пенсионер
	                Ежемесячный доход
